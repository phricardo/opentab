document.addEventListener('DOMContentLoaded', () => {
  ...
  const STORAGE_KEYS = ['searchTheme', 'searchLanguage', 'searchEngine', 'searchLogoMode', 'searchLogoText', 'searchPrimaryButtonColor'];
  ...
  const messages = {
    ...
    'pt-BR': {
      ...
      openSourceCredit: 'Este projeto \u00e9 de c\u00f3digo aberto e est\u00e1 dispon\u00edvel em {repo}',
      openSourceRepoLabel: 'phricardo/opentab'
    }
  };
  ...
  function storageSet(values, cb) {
    if (ext && ext.storage && ext.storage.sync) {
      if (typeof browser !== 'undefined') {
        ext.storage.sync.set(values).then(() => { if (cb) cb(); }).catch(() => { if (cb) cb(); });
      } else {
        ext.storage.sync.set(values, () => { if (cb) cb(); });
      }
    } else if (cb) {
      cb();
    }
  ...
  function readLocalShortcuts() {
    try {
      if (!hasLocalShortcutsValue()) return normalizeShortcuts(DEFAULT_SHORTCUTS);
      return normalizeShortcuts(JSON.parse(localStorage.getItem(SHORTCUTS_KEY) || '[]'));
    } catch (e) {
  ...
  function readQueryHistory() {
    try {
      ...
      return parsed
        .map((item) => {
          const query = String(item && item.query ? item.query : '').trim().replace(/\s+/g, ' ');
          const normalized = normalizeQuery(item && item.normalized ? item.normalized : query);
  ...
  function rememberQuery(query) {
    ...
    const now = Date.now();
    const existingIndex = queryHistory.findIndex((item) => item.normalized === normalized);
  ...
  function getQuerySuggestions(query) {
    ...
    return Array.from(candidates.values())
      .map((item) => {
        ...
        return {
          ...item,
          score: (isPrefixMatch ? 1000 : 0) + sourceScore + frequencyScore * 10 + recencyScore
        };
      })
      .sort((a, b) => b.score - a.score || b.lastUsed - a.lastUsed || a.query.localeCompare(b.query))
  ...
  function updateClock() {
    ...
    if (clockWidget) {
      clockWidget.setAttribute('aria-label', text.clockLabel + ': ' + formattedTime + ', ' + clockDate.textContent);
    }
  }
  ...
  function applySearchEngineIcon() {
    ...
    searchEngineIcon.dataset.engine = preferences.searchEngine === 'google-br' ? 'google' : 'duckduckgo';
  }
  ...
  function applyLanguage() {
    ...
    devCreditText.textContent = text.openSourceCredit.replace('{repo}', text.openSourceRepoLabel);
    devCredit.setAttribute('aria-label', text.openSourceCredit.replace('{repo}', text.openSourceRepoLabel));
  ...
  function renderShortcuts() {
    ...
    for (let index = 0; index < MAX_SHORTCUTS; index += 1) {
      ...
      item.className = shortcut ? 'shortcut-card is-filled' : 'shortcut-card is-empty';
      item.type = 'button';

      if (shortcut) {
        item.setAttribute('aria-label', text.openShortcut.replace('{name}', shortcut.label));
        ...
        removeButton.setAttribute('aria-label', text.removeShortcut.replace('{name}', shortcut.label));
  ...
  function openShortcutModal() {
    shortcutModalTrigger = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    shortcutForm.reset();
  ...
  function closeShortcutModal() {
    ...
    if (shortcutModalTrigger && typeof shortcutModalTrigger.focus === 'function') {
  ...
  function keepFocusInShortcutModal(event) {
    ...
    const focusableElements = Array.from(shortcutModal.querySelectorAll('button, input, [href], select, textarea, [tabindex]:not([tabindex="-1"])'))
  ...
  function removeShortcut(index) {
    saveShortcuts(shortcuts.filter((shortcut, shortcutIndex) => shortcut && shortcutIndex !== index));
  }
  ...
  function setActiveSuggestion(index) {
    ...
    if (activeSuggestionIndex >= 0 && searchSuggestions.children[activeSuggestionIndex]) {
      input.setAttribute('aria-activedescendant', searchSuggestions.children[activeSuggestionIndex].id);
    } else {
      input.removeAttribute('aria-activedescendant');
    }
  }
  ...
  storageGet([SHORTCUTS_KEY], (result) => {
    if (!result || !Object.prototype.hasOwnProperty.call(result, SHORTCUTS_KEY)) {
  ...
  shortcutModal.addEventListener('click', (event) => {
    if (event.target && event.target.hasAttribute('data-close-shortcut-modal')) {
      closeShortcutModal();
    }
  });
  ...
  input.addEventListener('keydown', (event) => {
    ...
    if (!searchSuggestions.hidden && event.key === 'ArrowDown') {
      event.preventDefault();
      const nextIndex = activeSuggestionIndex < visibleSuggestions.length - 1 ? activeSuggestionIndex + 1 : 0;
    ...
    if (!searchSuggestions.hidden && event.key === 'ArrowUp') {
      event.preventDefault();
      const nextIndex = activeSuggestionIndex > 0 ? activeSuggestionIndex - 1 : visibleSuggestions.length - 1;
      setActiveSuggestion(nextIndex);
      return;
    }

    if (!searchSuggestions.hidden && event.key === 'Enter' && activeSuggestionIndex >= 0) {
      event.preventDefault();
